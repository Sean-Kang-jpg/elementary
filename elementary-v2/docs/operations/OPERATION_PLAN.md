...
- [ ] Generalize `fetch_schoolinfo_2026.py`, K-apt collection, school/apartment builders, `build_operational_masters.py`, and upload manifests to accept explicit region/city scopes.
...
- [ ] Source field values for this scope: K-apt `시도`/`시군구` spellings, education-office name, legal-dong code prefix, and whether renamed-region values appear.
...
| Rollback | Per-region row counts for every operational table are recorded before upload. Uploads are scoped so one wave can be deleted by region/city scope and a Serving refresh restores the pre-load counts exactly. Capital-region counts (2,260 schools, 20,164 complexes, 20,891 Serving rows, unless a recorded recurring run changed them) are unchanged after every wave. A gate that fails after upload triggers rollback before investigation. |
...
| I-20 | Handled in source layer; naming open | 광주시 and 전라남도 merged on 2026-07-01. K-apt's `시도` column and 법정동주소 already write the merged value (`전남광주통합특별시`, 1,710 complexes) while its own road addresses, the school standard data, and the 학구도 polygons still write both names separately. The merged area carries two address depths (광주 has no city level, 전남 does), so both halves stay separate registry regions and `merged_source_regions` + `resolve_source_region()` translate the source value by 시군구 or road address; all 21,712 K-apt rows resolve into exactly 17 regions. Still open: the user-facing region name (K-apt writes 전남광주통합특별시, the reported administrative name is 전라남도광주특별시/특례시), and the plan for when school and school-zone sources publish merged naming. |
