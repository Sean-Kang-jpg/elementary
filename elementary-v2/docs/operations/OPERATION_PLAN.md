...
| I-21 | Open | The portable read-only build is not hermetic on the ETL workstation. `build_apartment_master_v1.latest_kapt_source()` globs `etl/local_outputs_20260320/` for the newest `kapt_basic_*.csv` instead of taking the file the bundle restored, so a machine running the daily collector builds from that day's snapshot: it resolved `kapt_basic_20260918.csv` while the bundle supplies `kapt_basic_20260904.csv`. The locked baseline therefore fails here (complex master 20,172 vs 20,164, Serving 20,900 vs 20,891) while a clean Actions runner still passes, since it only has the bundle file. Take the K-apt source from the restored manifest before relying on this build as a regression gate, and do not relock the baseline until it is hermetic. |
